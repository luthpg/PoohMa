...
- **問題**: CSVインポートテスト（`e2ee-seed-import.spec.ts`）など、数十件のレコードに対してクライアント側 Web Crypto 暗号化、外部 OGP フェッチ、形態素解析（ルビ取得）を順次実行する巨大なジャーニーテストでは、デフォルトタイムアウト（180秒）を超えてしまうことがある。
